...
product_revenue_analysis = pd.DataFrame({'total_revenue': product_totals})
...
for category in avg_units_by_category.index:
    ...
    print(category + ": Avg Units: " + "{:.1f}".format(avg_units) + ", Avg Price: $" + "{:.2f}".format(avg_price))

# 7.4 Display promotion effectiveness
print("\nPromotion Effectiveness:")
print("- Promoted Items Avg Sale: $" + "{:,.2f}".format(promo_comparison['promo_avg_sales']))
print("- Non-Promoted Items Avg Sale: $" + "{:,.2f}".format(promo_comparison['no_promo_avg_sales']))
print("- Revenue from Promotions: $" + "{:,.2f}".format(promo_comparison['promo_total_revenue']))

# 7.5 Report on data quality issues
columns_with_missing = missing_counts[missing_counts > 0].index.tolist()
total_missing = missing_counts.sum()
print("\nData Quality Report:")
print("- Missing Values Found:", columns_with_missing)
print("- Total Missing Entries:", int(total_missing))

# 7.6 Provide three key business recommendations based on the analysis
print("\nKEY BUSINESS RECOMMENDATIONS:")
print("""
1. FOCUS MARKETING INVESTMENT ON NORTH AMERICA AND EUROPE:
   North America and Europe are the top two revenue-generating regions.
   GlobalTech should prioritize inventory stocking and targeted marketing
   campaigns in these regions to capitalize on existing strong demand,
   especially for high-value categories like Computers and Smartphones.

2. LEVERAGE PROMOTIONS MORE STRATEGICALLY:
   The promotion comparison shows that promoted items generate different
   average sale values than non-promoted ones. Management should analyze
   which specific product categories benefit most from promotions (e.g.,
   Accessories and Audio) and run targeted promotional campaigns to drive
   volume without sacrificing margin on already high-performing products
   like LaptopPro and PhoneX.

3. ADDRESS MISSING DATA IN UNIT_PRICE AND UNITS COLUMNS:
   Two columns — Unit_Price and Units — contain missing values, which can
   skew revenue calculations and average pricing analysis. GlobalTech should
   implement stricter data entry validation at the point of sale to ensure
   complete records, and retroactively investigate and fill in the missing
   entries where possible to improve the accuracy of future analyses.
""")